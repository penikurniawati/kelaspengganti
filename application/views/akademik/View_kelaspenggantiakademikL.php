...
function hari_pada_tanggal($date){
	if ($date=="") {
		...
		$indeks=0;
		$datasesi = array();
		foreach ($tampilWaktuAkademik as $dataWaktuAkademik) {
			# code...
			// $s[$dataWaktuAkademik][$indeks]=
			array_push($datasesi,$dataWaktuAkademik->jam);
			array_push($s,$dataWaktuAkademik->jam);
			$s[$dataWaktuAkademik->jam]=array();
			array_push($s_block,$dataWaktuAkademik->jam);
			$s_block[$dataWaktuAkademik->jam]=0;
		...
		//foreach untuk misal minggu tenang, yang ditampilkan hanya yang jadwal pengganti aja
		...
		if($cekTahunAjaran->tgl_mulai_minggutenang <= $tanggal_input && $cekTahunAjaran->tgl_terakhir_minggutenang >= $tanggal_input){
			...
			foreach ($tampilRuangAkademik as $ruang) {
				...
				foreach ($datasesi as $sesi) {
					...
					if (!empty($cek_kelas_pengganti_hadir)) {
						...
						//jika ada, maka sesi itu diblok untuk dosen tsb
					...
					//cek apakah pada cell ini sesinya telah diblok? jika tidak, tampilkan ruangan 
					if ($s_block[$sesi] != 1) {
			...
			foreach ($tampilRuangAkademik as $ruang) {
				...
				# code...
					...
					//cek apakah pada cell ini sesinya telah diblok? jika tidak, maka tampilkan ruangan tersedia
						if ($s_block[$sesi] != 1) {
												...
												;}
													...
													;}
														...
														;}
																		...
																		# code...
																			...
																			<td style="padding: 0">
																				<center>
																					...
																					// gak ada jadwal
																					// cek apakah pada ruangan dan sesi ini (cell ini) ada kelas pengganti hadir
																					$cek_kelas_pengganti_hadir2 = $this->JadwalAkademikM->getKelasPenggantiHadirCustom($ruang->namaRuang, $sesi2, $tgl_absen);
															...
															<?php
																	;} // akhir dari segalanya wkwkw, engga , ini akhir dari minggu tenang
																	else{
																		?>
																		<thead>
																			<tr>
																...
																<div class="row">
																	<div class="col-md-12">
																		<div class="form-group">
																			...
																			<?php 
																			//echo $sesi;
																			$pertemuan = $this->JadwalAkademikM->getPertemuan($kelas[0]->id_kelas,$tgl, '09.00 - 10.40');
																			if(!empty($pertemuan)){
																				$id_pertemuan = $pertemuan[0]->id_pertemuan;
																			}else{
																				$id_pertemuan=NULL;
																			}
																			?>
