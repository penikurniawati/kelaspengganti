<?php
class JadwalAkademikM extends CI_Model{

  	public function getJadwalAkademik(){
    ...
    //Yang dipakai ini
    public function getJadwalAkademikW($hari, $waktu, $ruangan){
      $this->db->select("k.id_kelas, p.keterangan, p.tgl_pertemuan AS tglPertemuan, p.status_pertemuan AS statusPertemuan, j.id_jadwal AS id, w.hari AS Hari, w.jam AS Waktu, mk.nama_MK AS Matkul, k.nama_kelas AS Kelas, d.nama_dosen AS Dosen, r.nama_ruang AS Ruang, ta.nama_ta AS Semester, p.sesi");
      $this->db->from("jadwal j");
      $this->db->join("kelas k","k.id_kelas = j.id_kelas");
      $this->db->join("mata_kuliah mk","mk.id_mk = k.id_matkul");
      $this->db->join("kelas_dosen kd","kd.id_kelas = k.id_kelas");
	...
	public function getKelasPengganti($id_kelas, $jam) {
		$this->db->select("*");
	...
	public function getKelasPenggantiCustom($id_kelas, $tgl) {
	...
	public function presensi($presensiInfo) {
		$this->db->trans_start();
		
		$this->db->insert('pertemuan',$presensiInfo);
		...
		public function presensiPengganti($presensiPenggantiInfo) {
		$this->db->trans_start();
		
		$this->db->insert('pertemuan',$presensiPenggantiInfo);
		$insert_id = $this->db->insert_id();
